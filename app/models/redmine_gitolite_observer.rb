class RedmineGitoliteObserver < ActiveRecord::Observer
...
#  end
  ...
  def update_repositories(object)
    case object
      when Repository then RedmineGitolite::update_repositories(object.project)
      when User then RedmineGitolite::update_repositories(object.projects) unless is_login_save?(object)
      when GitolitePublicKey then RedmineGitolite::update_repositories(object.user.projects)
      when Member then RedmineGitolite::update_repositories(object.project)
      when Role then RedmineGitolite::update_repositories(object.members.map(&:project).uniq.compact)
    end
  end
